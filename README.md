...
This is the application for the LCP Software
...
```bash
mkvirtualenv lcpapp -p python3.11
...
Create a database named `lcpapp`.
...
```bash
celery -A lcpapp worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A lcpapp worker -l INFO -B --pool=solo
```
